...
from pokemon import Pokemon, PokemonStat, Ability, Move
...
class Pokedex:
        ...
        if not self.request.expanded:
            moves = [
                Move(name=d["move"]["name"],
                     ...
                     url=d["move"]["url"] if self.request.expanded else None)
                for d in data]
            return moves
        else:
    ...
    async def create_pokemon(self, data: dict) -> Pokemon:
        ...
        """
        stat_list = await self.create_pokemon_stat_list(data["stats"])
        ability_list = await self.create_pokemon_ability_list(data["abilities"])
        move_list = await self.create_pokemon_move_list(data["moves"])
